...
function createMockFetch(responses: Array<{ status: number; body?: unknown } | "network-error">) {
  let callCount = 0;
  ...
  return { fn: fn as typeof globalThis.fetch, get callCount() { return callCount; } };
}
...
describe("fetchWithRetry", () => {
  it("returns immediately on 200", async () => {
    ...
    const res = await fetchWithRetry("http://x", {}, { ...baseOpts, fetchFn: mock.fn });
  ...
  it("throws after exhausting retries on network error", async () => {
    const mock = createMockFetch(["network-error", "network-error", "network-error"]);
    await assert.rejects(() => fetchWithRetry("http://x", {}, { ...baseOpts, fetchFn: mock.fn }));
  ...
  it("calls onError on each failed attempt", async () => {
    ...
    const mock = createMockFetch([{ status: 503 }, { status: 200 }]);
    await fetchWithRetry("http://x", {}, { ...baseOpts, fetchFn: mock.fn, onError: (e) => errors.push(e) });
